...
class ProcessBuilder {
    ...
    build(){
        ...
        logger.info('Usage de liteloader:', this.usingLiteLoader)
        this.usingFabricLoader = this.server.modules.some(mdl => mdl.rawModule.type === Type.Fabric)
        logger.info('Usage de fabric:', this.usingFabricLoader)
        ...
        logger.info('Arguments lors du lancement:', args)
        ...
        child.on('close', (code, signal) => {
            ...
            fs.remove(tempNativePath, (err) => {
                if(err){
                    logger.warn('Erreur pendant la suppresion du fichier temporaire', err)
                } else {
                    logger.info('Dossier temporaire supprimé avec succes.')
                }
            })
        })
