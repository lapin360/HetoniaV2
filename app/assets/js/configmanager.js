...
const dataPath = path.join(sysRoot, '.galaxyquest')
...
exports.load = function(){
    ...
    if(doLoad){
        ...
        try {
            ...
            logger.error(err)
            logger.info("Le fichier de configuration contient du JSON mal formé ou est corrompu.")
            logger.info("Création d'un nouveau fichier de configuration.")
            fs.ensureDirSync(path.join(configPath, '..'))
            config = DEFAULT_CONFIG
            exports.save()
        }
    ...
    logger.info('Chargé avec succes')
}
